...
resource "configcat_setting" "my_third_ff" {
  config_id    = configcat_config.my_config.id
  key          = "MyThirdFF"
  name         = "My Third Feature Flag"
  order        = 2
  setting_type = "string"
}

# PROD
resource "configcat_setting_value_v2" "my_third_ff_prod" {
  environment_id = data.configcat_environments.prod_environment.environments.0.environment_id
  setting_id     = configcat_setting.my_third_ff.id
...
# FORNO
resource "configcat_setting_value_v2" "my_third_ff_forno" {
...
# STAGING
resource "configcat_setting_value_v2" "my_third_ff_staging" {
